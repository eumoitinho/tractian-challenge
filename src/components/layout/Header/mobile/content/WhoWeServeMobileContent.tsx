  ...
  return (
    <div className="py-4 px-4 space-y-4">
      <div className="space-y-3">
        <p className="uppercase text-slate-500 text-sm">{industriesData.byRoleLabel}</p>
        {industriesData.roles.map((role, i) => (
          ...
          >
            <IconBox src={`${WHO_ICON_PATH}/who-we-serve-by-role.svg`} size={16} boxClass="w-8 h-8 bg-white" />
            <span className="text-sm text-slate-500">{role.label}</span>
          </a>
        ))}
      </div>
      <div className="space-y-3">
        <p className="uppercase text-slate-500 text-sm">{industriesData.bySectorLabel}</p>
        {industriesData.sectors.map((sector, i) => (
          ...
          >
            <IconBox src={`${WHO_ICON_PATH}/who-we-serve-${sector.icon}.svg`} size={16} boxClass="w-8 h-8 bg-white" />
