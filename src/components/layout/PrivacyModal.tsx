...
const CATEGORIES: CookieCategory[] = [
  { key: "functional", defaultOn: true },
  { key: "strictlyNecessary", alwaysActive: true },
  { key: "targeting", defaultOn: true },
  { key: "performance", defaultOn: true },
];
...
function ToggleSwitch({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className="relative inline-flex w-[43px] h-[23px] shrink-0 cursor-pointer rounded-full border border-white transition-colors"
      style={{ backgroundColor: checked ? "rgb(70, 130, 84)" : "rgb(148, 163, 184)" }}
    >
      <span
        className="inline-block w-[18px] h-[18px] rounded-full bg-white transition-transform"
        style={{
...
export function PrivacyModal({ isOpen, onClose }: PrivacyModalProps) {
  ...
  const [toggles, setToggles] = useState<Record<string, boolean>>(() => {
    ...
    CATEGORIES.forEach((cat) => {
      initial[cat.key] = cat.alwaysActive || cat.defaultOn || false;
    });
    return initial;
  });
  ...
  const handleRejectAll = () => {
    ...
    CATEGORIES.forEach((cat) => {
      newToggles[cat.key] = cat.alwaysActive || false;
    });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between shrink-0" style={{ padding: "10px 0 10px 30px" }}>

          <svg className="w-28 h-6" fill="rgb(37, 99, 235)" viewBox="0 0 177 25">
            <path
          ...
          <button
            onClick={onClose}
            className="w-11 h-11 flex items-center justify-center cursor-pointer text-[#333] font-bold"
            aria-label="Close"
          >
        ...
        <div
          ...
          style={{ padding: "0 30px 0 30px", margin: "1px 3px 0 0" }}
        >

          <h2 className="text-base font-bold leading-6 text-slate-700" style={{ margin: "10px 0" }}>
            {t("title")}
          </h2>


          <p className="text-[13px] leading-[19.5px] text-slate-700 mb-6">
            {t("description")}
          </p>
          ...
          <a
            ...
            className="text-blue-800 text-[13px] underline mb-6 inline-block"
          >
            {t("moreInfo")}
          </a>


          <h3 className="text-base font-bold leading-[20.8px] text-slate-700 pb-[10px] mt-4">
            {t("manageTitle")}
          </h3>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    <svg
                      className={`w-3 h-3 transition-transform text-slate-500 ${expandedCategory === cat.key ? "rotate-90" : ""}`}
                      viewBox="0 0 12 12"
                      fill="none"
                    >
                      <path d="M4 2L8 6L4 10" stroke="currentColor" strokeWidth="1.5" />
                    </svg>
                    <h4 className={`text-sm text-slate-700 ${cat.alwaysActive ? "font-bold" : "font-semibold"}`}>
                  ...
                  <div className="flex items-center gap-2">
                    {cat.alwaysActive ? (
                      <span className="text-[13px] font-bold" style={{ color: "rgb(56, 96, 190)" }}>
                        {t("alwaysActive")}
                      </span>
                    ) : (
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-700" style={{ paddingRight: 5 }}>
                          {toggles[cat.key] ? t("active") : t("inactive")}
                        </span>
                        <ToggleSwitch
                          checked={toggles[cat.key]}
                          onChange={(v) => setToggles((prev) => ({ ...prev, [cat.key]: v }))}
                        />
                      </div>
                    )}
                  </div>
                ...
                {expandedCategory === cat.key && (
                  <div className="pb-3 pl-5">
                    <p className="text-[13px] leading-[19.5px] text-slate-700">
